...
class LeaveEncashmentExternalRepository {
    static async fetchLeaveEncashment(employee_code: EmployeeCodeAttributes): Promise<LeaveEncashmentResponse> {
        try {
            const data = await RequestHandler.post<LeaveEncashmentResponse, EmployeeCodeAttributes>(
                ERPAPI.fetchLeaveEncashment,
                employee_code
            );
            return data;
        } catch (error) {
    ...
    static async applyLeaveEncashment(payload: LeaveEncashmentWithPay): Promise<LeaveEncashmentResponse> {
        try {
            ...
            return await RequestHandler.post<LeaveEncashmentResponse, LeaveEncashmentWithEncash>(
                ERPAPI.applyLeaveEncashment,
                leaveEncashmentPayload
            );
        } catch (error) {
            console.error("Error applying leave encashment:", error);
            throw error;
        }
    }
    ...
    static async fetchApprovalLeaveEncashment(employee_code: EmployeeCodeAttributes): Promise<LeaveEncashmentResponse> {
        try {
            return await RequestHandler.post<LeaveEncashmentResponse, EmployeeCodeAttributes>(
                ERPAPI.leaveEncashmentApproval,
                employee_code
            );
        } catch (error) {
            console.error("Error fetching approval leave encashment:", error);
            throw error;
        }
    }
    ...
    static async approveLeaveEncashment(payload: LeaveApprovedAttributes): Promise<LeaveEncashmentResponse> {
        try {
            return await RequestHandler.post<LeaveEncashmentResponse, LeaveApprovedAttributes>(
                ERPAPI.leaveEncashmentApproved,
                payload
            );
        } catch (error) {
